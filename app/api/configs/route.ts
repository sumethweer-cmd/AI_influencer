    ...
    try {
        ...
        if (!data || data.length === 0) {
            const defaults = [
                ...
                { key_name: 'GEMINI_MODEL_NAME', key_value: 'gemini-3.1-flash-lite', description: '🤖 รุ่นของ Gemini ที่ต้องการใช้งาน (แนะนำ: gemini-3.1-flash-lite)', is_secret: false },
  ...
  "caption_draft": "..."
            ...
            if (!hasModelConfig) {
                await supabaseAdmin.from('system_configs').insert({
                    ...
                    key_value: 'gemini-3.1-flash-lite',
                    description: '🤖 รุ่นของ Gemini ที่ต้องการใช้งาน (แนะนำ: gemini-3.1-flash-lite)',
                    is_secret: false
                })
                // Fetch again
